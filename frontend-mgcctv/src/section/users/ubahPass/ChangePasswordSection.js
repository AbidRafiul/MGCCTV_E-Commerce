...
import { KeyRound, LoaderCircle, ShieldCheck, Home, ChevronRight } from "lucide-react";
      ...
      <div className="mx-auto max-w-5xl relative">
        ...
        <div className="relative z-10 mb-10 px-2 sm:px-4 flex flex-col items-start gap-4">
          ...
          <nav className="flex items-center text-xs sm:text-sm font-bold text-slate-500 bg-white/80 px-4 py-2.5 rounded-full backdrop-blur-md ring-1 ring-slate-200 shadow-sm w-full sm:w-auto overflow-x-auto [&::-webkit-scrollbar]:hidden mt-2">
            ...
            <Link href="/ubah-password" className="hover:text-blue-600 transition-colors shrink-0">
              Ubah Password
            </Link>
          </nav>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)] lg:items-start">
          ...
          <div className="flex-1">
            <div className="overflow-hidden rounded-[28px] bg-white shadow-[0_18px_50px_rgba(15,23,42,0.10)]">
              ...
              {error ? (
                <div className="mx-4 mt-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 md:mx-5">
                  {error}
                </div>
              ) : null}

              {success ? (
                <div className="mx-4 mt-4 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700 md:mx-5">
                  {success}
                </div>
              ) : null}

              <form onSubmit={handleSubmit} className="space-y-6 rounded-b-[28px] border-t border-slate-200 bg-white p-4 md:p-5">
