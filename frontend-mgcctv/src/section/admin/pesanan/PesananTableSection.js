...
import {
  ...
  STATUS_OPTIONS,
  PAYMENT_META,
} from "@/hooks/admin/pesanan/usePesanan";
...
function PaymentBadge({ status }) {
  ...
  return (
    <div
      className={`flex items-center justify-center gap-1 px-2 py-0.5 rounded-full w-fit mt-1.5 ${meta.badgeClass}`}
    >
      <Icon size={10} />
  ...
  return (
    <div className="relative flex items-center gap-2">
      {primaryAction ? (
        <button
          onClick={() =>
            onChangeStatus(order.id_pesanan, primaryAction.nextStatus)
          }
          disabled={isUpdating}
          className={`flex items-center gap-1.5 rounded-xl px-3.5 py-2 text-[11px] font-bold transition-all disabled:opacity-70 ${primaryAction.className}`}
        >
          {isUpdating ? (
      ...
      <button
        ...
        disabled={isUpdating}
        className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3 py-2 text-[11px] font-bold text-slate-600 shadow-sm transition-colors hover:bg-slate-50 disabled:opacity-70"
      >
            ...
            {STATUS_OPTIONS.map((option) => {
              ...
              return (
                <button
                  ...
                  disabled={isUpdating}
                  className={`flex w-full items-center justify-between rounded-xl px-3 py-2 text-xs font-semibold transition-colors disabled:opacity-70 ${isActive ? "bg-blue-50 text-blue-700" : "text-slate-600 hover:bg-slate-50"}`}
                >
    ...
    <>
      <div className="p-4 pb-0 flex items-center justify-between">
        <h2 className="text-sm font-bold text-[#0C2C55] flex items-center gap-2">
          <Package size={16} className="text-slate-400" /> Daftar Pesanan
        </h2>
        ...
        <>
          ...
          <div className="p-4 border-t border-slate-200 flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-50/30">
            <p className="text-[11px] text-slate-500">
              Menampilkan{" "}
              <span className="font-bold">{paginatedOrders.length}</span> dari{" "}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                (page) => (
                  <button
                    ...
                    className={`w-7 h-7 flex items-center justify-center rounded text-[11px] font-bold ${currentPage === page ? "bg-blue-600 text-white" : "border border-slate-200 text-slate-600 hover:bg-slate-50"}`}
                  >
                    {page}
                  </button>
                ),
