...
export default function Navbar() {
  ...
  useEffect(() => {
    ...
    const syncCartCount = async () => {
      const token = localStorage.getItem("token");
      if (token) {
        try { setCartCount(await getCartCount()); } catch (error) { setCartCount(0); }
      } else { setCartCount(0); }
    };
    ...
    const fetchProfile = async () => {
      ...
      if (!token) return;
      try {
        const res = await fetch(`${AUTH_API_URL}/profile`, { headers: { Authorization: `Bearer ${token}` } });
        const data = await res.json();
        if (res.ok) { setProfile(data.user || null); } else { setProfile(null); }
  ...
  return (
    <>
      <nav suppressHydrationWarning className={`fixed top-0 w-full z-[100] transition-all duration-300 font-sans ${isScrolled ? "bg-white/90 backdrop-blur-lg shadow-sm border-b border-slate-200/50 py-3" : "bg-white py-4 md:py-5"}`}>
        <div className="w-full px-5 md:px-8 xl:px-12 2xl:px-20 flex justify-between items-center">
          ...
          <div className="flex items-center gap-4 xl:gap-8 z-50">
            ...
            <div className="hidden lg:flex items-center gap-2">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} className="px-4 py-2 rounded-full text-slate-600 font-bold hover:bg-slate-50 hover:text-blue-600 transition-colors whitespace-nowrap">{link.label}</Link>
              ))}
            </div>
          </div>
          ...
          <div className="flex justify-end items-center gap-3 xl:gap-5">
            ...
            <div className="hidden lg:flex items-center gap-2 shrink-0">
              ...
              <button type="button" onClick={() => router.push("/keranjang")} className="relative p-2.5 rounded-full text-slate-600 bg-slate-100 hover:bg-blue-50 hover:text-blue-600 transition-colors shrink-0">
                ...
                <AnimatePresence>
                  {cartCount > 0 && <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="absolute -right-1 -top-1 flex h-4 min-w-[16px] items-center justify-center rounded-full bg-red-500 px-1 text-[9px] font-bold leading-none text-white shadow-sm ring-2 ring-white">{cartCount > 99 ? "99+" : cartCount}</motion.span>}
                </AnimatePresence>
              </button>
            </div>
              ...
              {isMounted && (!isLogin ? (
                ...
                <div className="flex items-center gap-2 p-1.5 pr-3 bg-slate-50 border border-slate-200 rounded-full shadow-sm hover:shadow-md transition-shadow ml-2">
                  <Link href="/profile" className="flex items-center gap-2.5 group">
                    <div className="w-8 h-8 bg-gradient-to-r from-blue-600 to-cyan-500 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-inner">{profileInitial}</div>
                    <div className="flex min-w-0 max-w-[120px] flex-col justify-center">
                      <span className="truncate font-bold text-slate-900 text-xs group-hover:text-blue-600 transition-colors">{profile?.nama || "User"}</span>
                    </div>
                  </Link>
                  <div className="w-px h-5 bg-slate-200 mx-1"></div>
                  <button type="button" onClick={() => setShowLogoutModal(true)} className="text-slate-400 hover:text-red-500 transition-colors" title="Logout"><LogOut size={16} /></button>
                </div>
              ))}
            </div>
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.3 }} className="absolute top-full left-0 w-full bg-white shadow-2xl border-t border-slate-100 lg:hidden overflow-hidden">
              <div className="px-6 py-6 flex flex-col gap-6">
                ...
                <div className="flex flex-col gap-1">
                  {navLinks.map((link) => (
                    <Link key={link.href} href={link.href} onClick={() => setIsMobileMenuOpen(false)} className="font-bold text-slate-700 text-base hover:text-blue-600 hover:bg-blue-50 transition-colors px-4 py-3 rounded-2xl">{link.label}</Link>
                  ))}
                </div>
          ...
          <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} transition={{ duration: 0.2 }} className="relative w-full max-w-[400px] bg-white rounded-3xl p-6 sm:p-8 shadow-2xl ring-1 ring-slate-100 overflow-hidden">
              ...
              <div className="flex flex-col items-center text-center">
                <div className="w-16 h-16 bg-red-50 text-red-500 rounded-full flex items-center justify-center mb-5 ring-8 ring-red-50/50"><LogOut size={28} className="ml-1" /></div>
