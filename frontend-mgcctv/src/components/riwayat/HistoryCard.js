...
const STATUS_META = {
  ...
  diproses: {
    label: "Diproses",
    statusClass: "border-sky-100 bg-sky-50 text-sky-700",
    icon: Package,
    accentClass: "from-sky-500/12 to-cyan-400/5",
  },
  dikirim: {
    label: "Dikirim",
  ...
  dibatalkan: {
    ...
    icon: XCircle,
    accentClass: "from-red-500/12 to-rose-400/5",
  },
};
...
const PAYMENT_STATUS_LABELS = {
  ...
  capture: "Pembayaran Terkonfirmasi",
  expire: "Kedaluwarsa",
  expired: "Kedaluwarsa",
  cancel: "Dibatalkan",
  deny: "Ditolak",
              ...
              <div className="relative z-10 flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
                <div>
                  ...
                  <p className="mt-2 max-w-2xl text-sm leading-relaxed text-blue-100/85">
                    Riwayat pesanan ini sekarang terhubung langsung ke transaksi dan detail transaksi dari database.
                  </p>
                </div>
              ...
              {isLoading ? (
                ...
                <div className="mt-6 rounded-[24px] border border-dashed border-slate-200 bg-slate-50 px-5 py-14 text-center">
                  ...
                  <p className="mt-2 text-sm text-slate-500">
                    Pesanan Anda akan muncul di sini setelah transaksi berhasil dibuat.
                  </p>
                </div>
              ) : (
                <div className="mt-6 space-y-4">
                  {orders.map((order) => {
                    ...
                    const paymentStatusLabel =
                      PAYMENT_STATUS_LABELS[order.status_bayar] || order.status_bayar || "-";
                    const productLabel =
                      order.items?.length > 1
                        ? `${firstProduct?.nama_produk || "Produk"} +${order.items.length - 1} item lainnya`
                        : firstProduct?.nama_produk || "Produk";

                    return (
                      <div
                        key={order.id_pesanan}
                        className={`overflow-hidden rounded-[24px] border border-slate-200 bg-gradient-to-br ${meta.accentClass} p-[1px]`}
                      >
                          ...
                          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                            <div className="flex items-start gap-4">
                              <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-2xl bg-slate-100 text-slate-500 shadow-inner overflow-hidden">
                                {firstProduct?.gambar_produk ? (
                                  <img
                                    src={firstProduct.gambar_produk}
                                    alt={productLabel}
                              ...
                              <div className="min-w-0">
                                <div className="flex flex-wrap items-center gap-2">
                                  ...
                                  <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-semibold text-slate-500">
                                    #{String(order.id_pesanan).padStart(4, "0")}
                                  </span>
                                </div>

                                <div className="mt-3 grid gap-2 text-sm text-slate-600 sm:grid-cols-3 sm:gap-6">
                                  <p>
                                    <span className="block text-[11px] uppercase tracking-[0.14em] text-slate-400">Tanggal Transaksi</span>
                                    <span className="mt-1 block font-medium text-slate-700">{formatDate(order.tanggal_transaksi)}</span>
                                  </p>
                                  <p>
                                    <span className="block text-[11px] uppercase tracking-[0.14em] text-slate-400">Jumlah Item</span>
                                    <span className="mt-1 block font-medium text-slate-700">
                                      {order.items.reduce((sum, item) => sum + Number(item.quantity || 0), 0)} item
                                    </span>
                                  </p>
                                  <p>
                                    <span className="block text-[11px] uppercase tracking-[0.14em] text-slate-400">Total Bayar</span>
                                    <span className="mt-1 block font-semibold text-slate-900">{formatCurrency(order.total_harga)}</span>
                                  </p>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex flex-col items-start gap-3 lg:items-end">
                              <span
                                className={`inline-flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-semibold ${meta.statusClass}`}
                              >
                                <StatusIcon size={14} />
                                {meta.label}
                              </span>
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setExpandedOrderId((prev) =>
                                    prev === order.id_pesanan ? null : order.id_pesanan,
                                  )
                                }
                                className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-4 py-2.5 text-xs font-semibold text-[#0C2C55] transition hover:bg-slate-200"
                              >
                                {isExpanded ? "Tutup Ringkasan" : "Lihat Ringkasan"}
                            ...
                            <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                              <div className="grid gap-3 md:grid-cols-4">
                                <div>
                                  ...
                                  <p className="mt-1 text-sm font-semibold text-slate-700">
                                    TRX-{String(order.id_pesanan).padStart(5, "0")}
                                  </p>
                                </div>
                                <div>
                                  <p className="text-[11px] uppercase tracking-[0.16em] text-slate-400">Tanggal Transaksi</p>
                                  <p className="mt-1 text-sm font-semibold text-slate-700">{formatDateTime(order.tanggal_transaksi)}</p>
                                </div>
                                <div>
                                  <p className="text-[11px] uppercase tracking-[0.16em] text-slate-400">Metode Bayar</p>
                                  <p className="mt-1 text-sm font-semibold text-slate-700">{order.metode_bayar || "-"}</p>
                                </div>
                                <div>
                                  <p className="text-[11px] uppercase tracking-[0.16em] text-slate-400">Status Bayar</p>
                                  <p className="mt-1 text-sm font-semibold text-slate-700">{paymentStatusLabel}</p>
                                </div>
                                <div>
                                  <p className="text-[11px] uppercase tracking-[0.16em] text-slate-400">Status Order</p>
                                  <p className="mt-1 text-sm font-semibold text-slate-700">{meta.label}</p>
                                </div>
                              </div>
                              ...
                              <div className="mt-4 space-y-3">
                                {order.items.map((item) => (
                                  <div
                                    key={item.id_detail_transcation}
                                    className="flex items-center justify-between gap-4 rounded-xl bg-white px-4 py-3"
                                  >
                                    <div className="min-w-0">
                                      <p className="text-sm font-semibold text-[#0C2C55]">{item.nama_produk}</p>
